...
export default function RoadmapVideoCard({ video, isEn, accentColor }: RoadmapVideoCardProps) {
    ...
    return (
        <div className="group flex-shrink-0 w-64 sm:w-72">
            <div className="relative rounded-xl overflow-hidden shadow-md bg-surface-dark aspect-video border border-white/5">
                {isPlaying ? (
                    <iframe
            ...
            <div className="mt-2 px-1">
                <p className="text-sm font-medium text-salt-white line-clamp-2 leading-snug">
                    {title}
                </p>
                <p className="text-xs text-caribbean-aqua/60 mt-0.5">
                    {video.channel}
                </p>
            </div>
        </div>
